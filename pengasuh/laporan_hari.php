...
$konteks = pengasuh_laporan_hari_konteks($pdo, $tanggal, count($detailKeg));
$telatData = pengasuh_laporan_hari_telat($pdo, $tanggal, $tkFilter);
...
$filterBase = static function (array $extra = []) use ($tanggal, $tingkatan, $kategori): string {
    ...
    return app_href('/pengasuh/laporan_hari.php?' . http_build_query($q));
};
...
$sdmHariBase = '/pengasuh/sdm_hari.php';

$pageTitle = 'Laporan Hari Ini';
$pageStylesheets = [app_asset_href('/assets/css/yayasan-portal.css'), app_asset_href('/assets/css/keaktifan-hari.css')];
$bodyClass = 'page-keaktifan-hari page-pengasuh-laporan';
...
<div class="yp-wrap kh-wrap">
    <div class="page-intro mb-3 d-flex flex-wrap justify-content-between gap-2">
        <div>
            <p class="page-intro-kicker mb-1">Pengasuh · Laporan Rekap Harian</p>
            ...
            <p class="text-muted mb-0 small">
                <?= htmlspecialchars((string) ($konteks['hari_label'] ?? '')) ?>, <?= htmlspecialchars($tglLabel) ?>
                <?php if (($konteks['hijri_label'] ?? '') !== ''): ?>
                    · <?= htmlspecialchars((string) $konteks['hijri_label']) ?>
                <?php endif; ?>
                · <?= (int) ($konteks['jumlah_kegiatan'] ?? 0) ?> kegiatan terjadwal
            </p>
            <?php if (($konteks['libur_label'] ?? '') !== ''): ?>
                <p class="small text-warning mb-0 mt-1"><i class="fa-solid fa-calendar-xmark me-1"></i><?= htmlspecialchars((string) $konteks['libur_label']) ?></p>
            <?php endif; ?>
        </div>
        <div class="d-flex flex-wrap gap-2 align-self-start">
            <a class="btn btn-sm btn-outline-secondary" href="<?= htmlspecialchars(app_href('/pengasuh/nilai_keaktifan.php')) ?>">
                <i class="fa-solid fa-star me-1"></i>Nilai Keaktifan
            </a>
            <a class="btn btn-sm btn-outline-secondary" href="<?= htmlspecialchars(app_href('/rekap/keaktivan_sdm.php?dari=' . urlencode($tanggal) . '&sampai=' . urlencode($tanggal))) ?>">
                <i class="fa-solid fa-users me-1"></i>SDM
            </a>
        </div>
    </div>
    ...
    <?php require __DIR__ . '/../includes/partials/laporan_hari_pelengkap.php'; ?>
    ...
    <section class="mb-4">
        ...
        <div class="row g-3">
            <div class="col-md-6">
                <a class="yp-sdm-card" href="<?= htmlspecialchars(app_href($sdmHariBase . '?role=pembimbing&tanggal=' . urlencode($tanggal))) ?>">
            ...
            <div class="col-md-6">
                <a class="yp-sdm-card" href="<?= htmlspecialchars(app_href($sdmHariBase . '?role=munawib&tanggal=' . urlencode($tanggal))) ?>">
                    <div class="yp-sdm-card__head">
