...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'save_pesantren' || $action === 'save_settings') {
        ...
        foreach ($pondokIdentityFields as $field) {
            ...
            if ($field === 'pwa_theme_color' || $field === 'pwa_background_color') {
                $val = $val !== '' ? pwa_brand_normalize_hex($val, '') : '';
            }
            save_setting($pdo, $field, $val);
        }
        ...
        if (isset($_FILES['logo_file']) && is_array($_FILES['logo_file']) && (int) $_FILES['logo_file']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($extension, $allowed, true)) {
                ...
                if (move_uploaded_file($tmpFile, $targetPath)) {
                    $logoRel = 'uploads/logos/' . $safeName;
                    $optimizedRel = logo_image_process_uploaded_logo($targetPath);
                    if ($optimizedRel !== null) {
                        $logoRel = $optimizedRel;
                    }
                    save_setting($pdo, 'logo_path', $logoRel);
        ...
        if (trim((string) app_setting($pdo, 'logo_path', '')) !== '') {
            ...
            pwa_brand_sync_theme_from_brand($pdo);
        }
    ...
    if ($action === 'save_wa_settings') {
        foreach ($pondokWaFields as $field) {
            if (!array_key_exists($field, $_POST)) {
                continue;
            }
