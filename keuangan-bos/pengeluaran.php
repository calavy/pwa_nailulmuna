...
<form method="post" class="card shadow-sm" style="max-width:42rem" id="form-pengeluaran-bos">
    ...
    <div class="card-body row g-3">
        ...
        <div class="col-12 d-none" id="field-pos-lain">
            <label class="form-label">Pos pengeluaran lain <span class="text-danger">*</span></label>
            <select name="pos_pengeluaran_id" class="form-select">
                <option value="">— pilih pos —</option>
                <?php foreach ($posRows as $pr): ?>
                    <option value="<?= (int) $pr['id'] ?>">
                        <?= htmlspecialchars((string) $pr['nama_pos']) ?> (<?= htmlspecialchars(bos_label_jenjang((string) ($pr['tag_jenjang'] ?? ''))) ?>)
                    </option>
                <?php endforeach; ?>
            </select>
            <p class="small text-muted mt-1 mb-0">Diposting ke COA 5199 — Beban Operasional Lain-lain.</p>
        </div>
        <div class="col-md-6">
