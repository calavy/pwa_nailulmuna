...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'setor_koperasi_multi') {
        ...
        set_flash($res['ok'] ? 'success' : 'warning', (string) ($res['message'] ?? 'Setor gagal.'));
    } elseif ($action === 'delete_debit_tx' && $isSuperAdmin) {
        $txId = (int) ($_POST['tx_id'] ?? 0);
        $res = cashless_koperasi_hapus_debit($pdo, $txId);
        set_flash($res['ok'] ? 'success' : 'warning', (string) ($res['message'] ?? 'Gagal menghapus.'));
    } elseif ($action === 'delete_debit_tx') {
        set_flash('error', 'Hanya admin super yang dapat menghapus transaksi.');
    }
                    ...
                    <tr>
                        ...
                        <td class="text-end">
                            <form method="post" class="d-inline" onsubmit="return confirm('Hapus transaksi ini?');">
                                ...
                                <input type="hidden" name="tx_id" value="<?= (int) ($tx['id'] ?? 0) ?>">
                                <input type="hidden" name="tanggal" value="<?= htmlspecialchars($tanggal) ?>">
