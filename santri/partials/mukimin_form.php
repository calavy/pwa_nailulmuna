...
$isEdit = $editRow !== null;
?>
<div id="mukimin-form-panel" class="mukimin-form-panel collapse<?= $showFormPanel ? ' show' : '' ?>">
    <div class="card shadow-sm border-0">
        <div class="card-body">
            <div class="d-flex justify-content-between align-items-start gap-2 mb-3">
                <h2 class="h5 mb-0"><?= $isEdit ? 'Ubah data mukimin' : 'Tambah data mukimin' ?></h2>
                <button type="button" class="btn btn-sm btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#mukimin-form-panel" aria-expanded="<?= $showFormPanel ? 'true' : 'false' ?>" aria-controls="mukimin-form-panel">Tutup</button>
            </div>
            <form method="post" class="d-grid gap-2">
                ...
                <div class="d-flex flex-wrap gap-2 pt-1">
                    ...
                    <?php if ($isEdit): ?>
                        <a class="btn btn-outline-secondary" href="<?= htmlspecialchars(mukimin_page_url(['edit' => ''], $filters ?? [])) ?>">Batal</a>
                    <?php else: ?>
                        <button type="button" class="btn btn-outline-secondary" data-bs-toggle="collapse" data-bs-target="#mukimin-form-panel">Batal</button>
                    <?php endif; ?>
                </div>
            </form>
        </div>
